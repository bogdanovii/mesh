...
class Setka:
    ...
    def __init__(self):#конструктор в си переведен
    ...
    def sdvig_v_sloe_XZ_coord_Z_do_ploskosti_Z(self, BoundLow_X, BoundUp_X, a, b, c, d):
        ...
        with open("p_to_c", "w") as file:
            for x in range(self.N_x):
                for y in range(self.N_y):
                    for z in range(self.N_z):
                        file.write(str(self.coord_of_Nodes_sdvig[x][y][z][0]) + "\n")
                        file.write(str(self.coord_of_Nodes_sdvig[x][y][z][1]) + "\n")
                        file.write(str(self.coord_of_Nodes_sdvig[x][y][z][2]) + "\n")
        file.close()

        #for x in range(self.N_x):
        #    for y in range(self.N_y):
        #        for z in range(self.N_z):
        #            datas['Coord_of_Nodes'].append({'x': self.coord_of_Nodes_sdvig[x][y][z][0], 'y': self.coord_of_Nodes_sdvig[x][y][z][1], 'z': self.coord_of_Nodes_sdvig[x][y][z][2]})
        #with open('resPySdvig', 'w') as outfile:
...
class Printer:
    ...
    def print_Paralepip(self):
        ...
        #with open('MyX_Y_Z.vtk', 'w') as my_f:#если надо в конце файла, то a
            ...
            #POINT_DATA n
            #Пишем число узлов в наш втк файл

            #my_f.write(' '.join(['POINTS', str(int(self.setka.Number_of_nodes)), 'double\n']))

        #with open('cppMyX_Y_Z.vtk', 'a') as my_f:
